	...
	if($result){
		if(isset($_FILES['image'])){
			if(isset($category['image'])){
				...
				echo '<script>alert("êtes vous sûre de vouloir suprimer l\'ancienne photo")</script>';
				...
				if ( in_array($my_file_extension , $allowed_extensions) ){
					...
					$resultUpdateImage = $query->execute(
						...
						echo '<script>alert("mauvaise extention")</script>';
					}
			}else{
					echo '<script>alert("pas d\'image dans la table category")</script>';
				}
		}else{
				echo '<script>alert("pas d\'image dans form")</script>';
			}
	}else{
			echo '<script>alert("article pas modifier")</script>';
		}
