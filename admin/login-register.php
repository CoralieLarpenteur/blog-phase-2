...
if(isset($_POST['register'])){
	if (!empty($_POST['firstname']) && !empty($_POST['email']) && !empty($_POST['password']) && !empty($_POST['password_confirm'])) {
		if ($_POST['password']==$_POST['password_confirm']) {
			$query = $db->prepare('INSERT INTO user (firstname, lastname, password, email, is_admin, bio) VALUES (?, ?, ?, ?, 0, ?)');
			$user = $query->execute(
			[
				$_POST['firstname'],
				$_POST['lastname'],
				...
				$query->execute();
				$newUser = $query->fetch();
				$_SESSION['user'] = $newUser['firstname'];
		}else {
